...
def OCR_Project(file_path):
    ...
    cv2.imwrite("D:\Coding\Python\CSE100 Project\inverted_image.png", inverted_image)
    ...
    cv2.imwrite('D:\Coding\Python\CSE100 Project\grayimage.png', grayimage)
    # display('D:\Coding\Python\CSE100 Project\grayimage.png')

    thresh, im_bw = cv2.threshold(grayimage, 127, 255, cv2.THRESH_BINARY)
    cv2.imwrite('D:\Coding\Python\CSE100 Project\im_bw.png', im_bw)
    ...
    cv2.imwrite("D:\Coding\Python\CSE100 Project\no_noise.png", no_noise)
    ...
    cv2.imwrite("D:\Coding\Python\CSE100 Project\eroded_image.png", eroded_image)
    ...
    cv2.imwrite('D:\Coding\Python\CSE100 Project\thickfont_img.png', dilated_image)
    ...
    def getSkewAngle(cvImage) -> float:
        ...
        cv2.imwrite("D:\Coding\Python\CSE100 Project\boxes_img.png", newImage)
    ...
    fixed = deskew(new)
    cv2.imwrite('D:\Coding\Python\CSE100 Project\rotated_fixed.png', fixed)
    ...
    cv2.imwrite("D:\Coding\Python\CSE100 Project\image_with_borders.png", image_with_border)
    ...
    import pytesseract  # Img --> Text
    import cv2  # For img processing
    im_path = file_path
    image = cv2.imread(im_path)  # storing img path in variable called image

    base_image = image.copy()
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)  # Converts colored img to grayscale
    cv2.imwrite("D:\Coding\Python\CSE100 Project\grayimage.png", gray)  # New file grayimage.png

    blur = cv2.GaussianBlur(gray, (7, 7), 0)  # Blurring image
    cv2.imwrite("D:\Coding\Python\CSE100 Project\blur.png", blur)

    thresh = cv2.threshold(blur, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)[1]
    cv2.imwrite("D:\Coding\Python\CSE100 Project\thresh.png", thresh)

    kernal = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 13))
    cv2.imwrite("D:\Coding\Python\CSE100 Project\kernal.png", kernal)

    dilate = cv2.dilate(thresh, kernal, iterations=1)
    cv2.imwrite("D:\Coding\Python\CSE100 Project\dilated_image.png", dilate)
    ...
    for c in cnts:
        ...
        cv2.imwrite("D:\Coding\Python\CSE100 Project\roi.png", roi)
        cv2.rectangle(image, (x, y), (x + w, y + h), (36, 255, 12), 2)

    cv2.imwrite("D:\Coding\Python\CSE100 Project\box_new.png", image)
    ...
    im_h, im_w, im_d = image.shape  # height, width, depth
    base_image = image.copy()

    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)  # Grayscale conversion
    blur = cv2.GaussianBlur(gray, (7, 7), 0)  # Blurring
    thresh = cv2.threshold(blur, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)[1]  # Threshold
    ...
    cv2.imwrite("D:\Coding\Python\CSE100 Project\output_image.png", image)
...
def perform_ocr():
    ...
    try:
        ...
        ocr_result = OCR_Project(filepath)
        # ocr_result = pytesseract.image_to_string(Image.fromarray(cv2.cvtColor(image, cv2.COLOR_BGR2RGB)))

        # Display the OCR result in the text area
        ocr_result_text.delete(1.0, tk.END)
        ocr_result_text.insert(tk.END, ocr_result)
...
select_button = tk.Button(file_frame, text="Select Image", command=select_image)
...
# Create a text area to display the OCR result
ocr_result_text = tk.Text(root, height=10, width=50)
ocr_result_text.pack(pady=5)
